import React, { useState } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { Eye, EyeOff, Github, Terminal, ArrowRight } from 'lucide-react';

interface Props { mode?: 'signin' | 'signup' }

const ACCENT = '#FF5C00';
const HAIRLINE = '#E5E5E5';

export function SignInPage({ mode = 'signin' }: Props) {
  const [showPassword, setShowPassword] = useState(false);
  const navigate = useNavigate();
  const isSignup = mode === 'signup';

  return (
    <div
      className="min-h-screen flex bg-[#FAFAFA] text-black selection:bg-[#FF5C00] selection:text-white"
      style={{ fontFamily: "'Space Grotesk', sans-serif" }}
    >
      {/* ── LEFT PANEL: Brand ── */}
      <section
        className="hidden lg:flex lg:w-1/2 relative flex-col justify-between p-16 bg-white border-r"
        style={{ borderColor: HAIRLINE }}
      >
        <Link to="/" className="relative z-10 flex items-center gap-2">
          <Terminal className="w-6 h-6" />
          <span className="font-bold text-xl tracking-tight">RepoHive</span>
        </Link>

        <div className="relative z-10 max-w-lg">
          <h1
            className="font-bold tracking-tighter leading-[1.05] mb-6"
            style={{ fontSize: 'clamp(2.5rem, 4.5vw, 4rem)' }}
          >
            All your repos.<br />Under your control.
          </h1>
          <p className="text-gray-500 text-lg max-w-md leading-relaxed font-medium">
            Paste any GitHub URL. Get AI-analyzed cards with scores, tags, and plain-English semantic search across your whole library.
          </p>
        </div>

        <div className="relative z-10 flex items-center gap-4">
          <div
            className="flex items-center gap-2 px-3 py-1 rounded-full border"
            style={{ borderColor: HAIRLINE }}
          >
            <span className="w-2 h-2 rounded-full bg-[#FF5C00] animate-pulse" />
            <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-gray-500">
              All systems operational
            </span>
          </div>
          <span className="text-[10px] font-mono uppercase tracking-widest text-gray-400">
            © {new Date().getFullYear()} RepoHive
          </span>
        </div>
      </section>

      {/* ── RIGHT PANEL: Form ── */}
      <main className="w-full lg:w-1/2 flex items-center justify-center p-6 bg-[#FAFAFA]">
        <div
          className="w-full max-w-md rounded-md p-10 bg-white border shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]"
          style={{ borderColor: '#000' }}
        >
          {/* Mobile logo */}
          <div className="lg:hidden mb-8">
            <Link to="/" className="flex items-center gap-2">
              <Terminal className="w-5 h-5" />
              <span className="font-bold text-lg tracking-tight">RepoHive</span>
            </Link>
          </div>

          <div className="mb-10">
            <h2 className="text-3xl font-bold tracking-tight mb-2">
              {isSignup ? 'Create your account' : 'Welcome back'}
            </h2>
            <p className="text-gray-500 text-sm font-medium">
              {isSignup ? 'Start organizing your repo intelligence.' : 'Sign in to access your library.'}
            </p>
          </div>

          {/* Social auth */}
          <div className="grid grid-cols-2 gap-4 mb-8">
            {[
              { icon: <GoogleIcon />, label: 'Google' },
              { icon: <Github className="w-5 h-5" />, label: 'GitHub' },
            ].map(({ icon, label }) => (
              <button
                key={label}
                className="flex items-center justify-center gap-3 px-4 py-2.5 rounded transition-colors border border-black hover:bg-gray-50"
              >
                {icon}
                <span className="text-xs font-bold uppercase tracking-wider">{label}</span>
              </button>
            ))}
          </div>

          {/* Divider */}
          <div className="relative flex items-center justify-center mb-8">
            <div className="w-full h-px" style={{ background: HAIRLINE }} />
            <span className="absolute px-4 text-[10px] font-mono uppercase tracking-[0.2em] bg-white text-gray-400">
              or with email
            </span>
          </div>

          {/* Form */}
          <form className="space-y-5" onSubmit={e => { e.preventDefault(); navigate('/app'); }}>
            <div>
              <label className="block text-[10px] font-mono font-bold uppercase tracking-widest mb-2 text-gray-500">
                Email
              </label>
              <input
                type="email"
                placeholder="you@company.com"
                className="w-full rounded px-4 py-3 text-sm outline-none transition-colors border bg-white placeholder-gray-400 focus:border-black"
                style={{ borderColor: HAIRLINE }}
              />
            </div>

            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="text-[10px] font-mono font-bold uppercase tracking-widest text-gray-500">
                  Password
                </label>
                {!isSignup && (
                  <a href="#" className="text-[10px] font-mono font-bold uppercase tracking-widest transition-colors hover:text-[#FF5C00]" style={{ color: ACCENT }}>
                    Forgot?
                  </a>
                )}
              </div>
              <div className="relative">
                <input
                  type={showPassword ? 'text' : 'password'}
                  placeholder="••••••••••••"
                  className="w-full rounded px-4 py-3 pr-12 text-sm outline-none transition-colors border bg-white placeholder-gray-400 focus:border-black"
                  style={{ borderColor: HAIRLINE }}
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-black transition-colors"
                >
                  {showPassword ? <EyeOff className="w-[18px] h-[18px]" /> : <Eye className="w-[18px] h-[18px]" />}
                </button>
              </div>
            </div>

            {!isSignup && (
              <div className="flex items-center gap-3 py-2">
                <input id="remember" type="checkbox" className="w-4 h-4 rounded accent-[#FF5C00]" />
                <label htmlFor="remember" className="text-xs text-gray-500 cursor-pointer">
                  Keep me signed in
                </label>
              </div>
            )}

            <button
              type="submit"
              className="w-full py-3.5 rounded font-bold text-sm bg-black text-white hover:bg-[#FF5C00] transition-colors flex items-center justify-center gap-2"
            >
              {isSignup ? 'Create account' : 'Sign in'} <ArrowRight className="w-4 h-4" />
            </button>
          </form>

          <div className="mt-10 text-center">
            <p className="text-sm text-gray-500">
              {isSignup ? 'Already have an account? ' : "Don't have an account? "}
              <Link
                to={isSignup ? '/sign-in' : '/sign-up'}
                className="font-bold hover:underline underline-offset-4 transition-colors"
                style={{ color: ACCENT }}
              >
                {isSignup ? 'Sign in' : 'Start free'}
              </Link>
            </p>
          </div>
        </div>
      </main>
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg className="w-5 h-5" viewBox="0 0 24 24">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
    </svg>
  );
}
